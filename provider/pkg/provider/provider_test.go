...
func readFileFromProviderResourceDir(t *testing.T, filename string) []byte {
	...
	b, err := os.ReadFile(filepath.Join("..", "..", "cmd", "pulumi-resource-doppler-native", filename))
	if err != nil {
...
func makeTestProvider(ctx context.Context, t *testing.T) pulumirpc.ResourceProviderServer {
	...
	_, err = p.Configure(ctx, &pulumirpc.ConfigureRequest{
		Variables: map[string]string{"doppler-native:config:apiKey": "fakeapikey"},
	})
...
func TestDiff(t *testing.T) {
	...
	resp, err := p.Diff(ctx, &pulumirpc.DiffRequest{Id: "", Urn: "urn:pulumi:some-stack::some-project::doppler-native:services:StaticSite::someResourceName", Olds: oldsStruct, News: newsStruct})
	assert.Nil(t, err)
...
func TestCreate(t *testing.T) {
	...
	_, err := p.Create(ctx, &pulumirpc.CreateRequest{
		Urn:        "urn:pulumi:dev::doppler-native-ts::doppler-native:services:WebService::webservice",
